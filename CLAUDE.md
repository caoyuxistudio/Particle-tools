...
- 规划里的 M0–M4 全部完成（2026-09-17）：引擎在 `packages/engine`（边界脚本 0 违规、jest 24 条）；schema 覆盖默认 config 的全部键；studio 与 V1 对等（三个 canvas 编辑器、演示模式、Perf / Gyro HUD、手指尾迹、视差、手机布局），harness `__st.report()` 75 条全绿；studio 已上线，仓库改名 Particle-tools，v2 合进 main。细节和判据在 `packages/studio/CLAUDE.md`「现状与下一步」和 §6。
...
- studio：dev 页面 console `await __st.report()`（75 条）；真实窗口 + `packages/editor/scripts/cdp-eval.mjs` 更可靠（方法见 `packages/editor/CLAUDE.md`「验证改动」，地址换成 5173）。
