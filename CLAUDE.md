...
- 控制台 harness `public/__ai-test.js`，当前基线 **361/361**（含 `noiseReport` 11、`colorInstanceReport` 36、`collisionReport` 10、`trailReport` 9、`stretchReport` 13、`aoReport` 8、`shadowReport` 10、`report` 35、`standaloneReport` 20、`touchReport` 9、`parallaxReport` 19、`videoReport` 30、`gizmoReport` 12、`playerReport` 41、`presentReport` 36、`frameReport` 28；`perfReport` 是测量不是断言，不计入）
...
- 手指把粒子扫到墙上不再弹飞：穿墙那段里手指推的部分钉在墙面、反射只反射粒子自己的运动，弹跳权重不压手指。改前一根 40 u/s 的手指撞墙反射出 154 u/s、之后一秒里慢慢衰减；现在 1.1。jest 加 2 条，`stretchReport` 加 2 条，基线 361/361。
