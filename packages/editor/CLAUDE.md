...
- **视口家具整修**（同日，作者用 studio 看空间时提的四条）：① 一片绿是四面碰撞面手柄叠出来的（每面一张 8% 的填充平面 + 35% 的网格），填充降到 2%、网格 18%；② 发射器形状辅助线在 GPU 路径上一直画在原点——粒子 mesh 的 `matrixWorldAutoUpdate` 是 false、transform 在 kernel 里应用，挂在它下面的子物体全在原点（V1 也是这样，只是没人注意）；studio 把它挂到容器上、按文档的 transform 摆放（`furniture.ts`），并且换成蓝紫色、加了发射方向箭头、四角括号、淡对角线和一个 EMITTER 标签（`shape-helper.ts`，V1 共用这套画法，位置的 bug V1 里还在）；③ 地面线框 `0x111111` → `0x242424`；④ 色源 debug 层的开关从 Helper 挪到 Particle Color Instance（`show source (debug)`），另外视口左下多了一排家具开关（emitter / walls / fields / source / axes，`viewport/Toolbar.svelte`），点的是同一批文档键。example-1-1 的 config 里 `showColorSourceDebug` 存的是 true，所以开机它是亮的，要默认关得改作品的 JSON。harness 加 4 条，38/38。
- **空间关系与结构线**（同日，第二批）：色源 debug 平面改为参与深度（`depthTest` 开、`depthWrite` 关）——画框挡得住它、它挡得住后面的，不再浮在整个空间上面；studio 的 `scene-decor.ts` 每帧给场景物体穿上编辑器专属的"装饰"（全部在家具层，输出、预览、player 不见）：每个 mesh 的边线（`EdgesGeometry`，和地面线框同一个灰 `0x242424`，稍微放大 0.3% 免得被面吃掉，几何变了就重建），相机一个线框机身（机身、镜头、两个片盘，看一眼就知道是相机），点光一个八面体加射线的灯泡，平行光一个朝向目标的太阳盘加一根世界空间的瞄准箭头（常驻，不用选中）；引擎多了 `getLiveObject(id)`。开机视角改成 `resetCamera()` 的 home view（45° 环绕、45° 仰角、装置框满），工具栏加 `reset view`。Perf / Gyro 两个 HUD 套上 token 样式（`ui/hud.css` 覆盖它们自注入的 CSS），每个浮窗右上角加了关闭键（HUD 的 DOM 是第一次打开才建的，用 MutationObserver 挂）。harness 加 7 条，45/45。
- **预览窗与右栏两个新标签**（同日）：预览窗（`world.ts`）改成一扇能拿的窗——拽任一边缘就是拉那条边（对边不动、按相机画幅保持比例，上下边通过高度算），拽内部就是整窗移动，红色把手删了，边框在指针下面变亮；位置存 `preview-offset`（相对自由区右上角的偏移，永远夹在自由区里），尺寸仍存 `preview-scale`；`__world` 多了 `getPreviewOffset / setPreviewOffset`。右栏现在四个标签 **particles / scene / pieces / textures**（默认宽 340 → 360）：pieces 是 V1 最左栏的 Examples + My Saved Configs（`library/ExamplesPanel.svelte`：存 / 载 / 删本浏览器里的作品，example 缩略图卡片一点就载）；textures 是 V1 的 Textures 面板移植（`library/TexturesPanel.svelte`：搜索、add image（缩到 1024 宽存 localStorage）、add video（文件进 IndexedDB）、按 URL 加视频、每条 use / in use / 删，内置的两幅画也列出来，terrain 贴图不列）。为此 examples 的清单和 saved-configs 的读写从 V1 的 UI 目录搬进引擎（`presets.ts`、`saved-configs.ts`，V1 的旧文件改成转发），色源的绑定统一走 `editors/open.ts` 的 `useColourSource(id)`。harness 加 6 条，51/51。
- **WIP-Test 删了**（同日，作者：打开会崩、不再需要）：从 `presets.ts` 的清单去掉，`public/examples/wip-test/` 整个文件夹删除；V1 的 `report` 改成列两个。WIP-Test-2 留作备用。
