...
- Timeline: drag the playhead (anywhere on the track that is not an end) and the simulation is brought to that frame — a chase by undrawn fixed steps, forward from where it is when it can, from the start of the range with nothing alive when going back, 14 ms of each animation frame so it is a fast-forward you watch (example-1-1, 500k particles: 300 frames in 1.8 s). A `length` field after fps; the track is the whole project. Pause now holds the particles themselves (the compute pass is skipped on a frame that did not step). Harness +4 (94).
