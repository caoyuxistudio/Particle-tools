...
- Timeline: the playhead can be taken by the hand — time goes where it is dragged and plays on from there; the particles are left as they are (no rewind, no re-run; a first version chased the target by re-simulating, which is not what the author wanted — it is in 022401d if an export ever needs it). A click on the bare track does nothing. A `length` field after fps; the track is the whole project. Pause holds the particles themselves (the compute pass is skipped on a frame that did not step). Harness 96.
- Pieces: thumbnails keep their camera's shape, so the example cards are laid out as two columns that fill downwards (CSS columns) instead of rows of 9:16; `__studio.capturePreview(width)` returns a piece's thumbnail as a data URL. Harness +3 (99).
