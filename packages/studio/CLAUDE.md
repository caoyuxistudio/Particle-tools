...
- **harness**：只在 dev bundle 里，页面 console 里 `await __st.report()`（94 条：boot、加载条、保存再读回、round-trip、schema 覆盖、live / rebuild 代价、家具、三个编辑器、演示、HUD、预览窗、面板）。真实窗口里跑更可靠，方法见 `packages/editor/CLAUDE.md` 的「验证改动」。
...
- **时间轴**（2026-09-18，`app/Timeline.svelte` + 引擎 `timeline.ts`）：底栏上面一条——播放 / 暂停 / 停止（回到范围开头并清空模拟）、当前帧号和时间码 `mm:ss:ff`、范围的两端（拖或者填数）、总帧数和秒数、fps（改了保持范围的秒数）、loop、restart（循环回到开头时模拟要不要从头来，默认不要：计数器转回去、粒子继续）、real time。**real time 关掉 = 每画一帧恰好步进 1/fps**，画得再慢也不跳帧，是慢动作而不是掉帧——序列帧导出要的就是这个。session 的时钟从此是时间轴的：`cycleData.elapsed` 和库的 `now` 是步长的累加（`simSeconds`），不读墙上时钟；暂停就是不步进。参数在 `_editorData.timeline`，动过才写进文档（不动就不出现在 COPY 里，round-trip 不变）。**拖播放头**（轨道上除两端以外的任何地方）= `session.seek(frame)`：粒子模拟只能一步步积出来，所以是"追"——目标在前面且模拟和时间轴还同步就从当前状态往前步进，否则回到范围开头清空重来；每个动画帧拿 14 ms 做不渲染的固定步长（1/fps，含 compute pass），然后照常画一帧，所以长距离是看得见的快进而不是卡死（example-1-1、50 万粒子：300 帧 1.8 秒）。拖动中不断改目标，追的过程跟着改。`length` 是整个项目的总帧数（默认 1200），轨道就是 0 … length，start / end 在里面选。暂停时 compute pass 也跳过（否则 kernel 会按上一步的 delta 继续推粒子，暂停停不住）。要瞬间回拖得靠快照，和种子、导出一起记在根目录 `BACKLOG.md`。
