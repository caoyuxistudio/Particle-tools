...
- **harness**：只在 dev bundle 里，页面 console 里 `await __st.report()`（99 条：boot、加载条、保存再读回、round-trip、schema 覆盖、live / rebuild 代价、家具、三个编辑器、演示、HUD、预览窗、面板）。真实窗口里跑更可靠，方法见 `packages/editor/CLAUDE.md` 的「验证改动」。
...
- **时间轴**（2026-09-18，`app/Timeline.svelte` + 引擎 `timeline.ts`）：底栏上面一条——播放 / 暂停 / 停止（回到范围开头并清空模拟）、当前帧号和时间码 `mm:ss:ff`、范围的两端（拖或者填数）、总帧数和秒数、fps（改了保持范围的秒数）、loop、restart（循环回到开头时模拟要不要从头来，默认不要：计数器转回去、粒子继续）、real time。**real time 关掉 = 每画一帧恰好步进 1/fps**，画得再慢也不跳帧，是慢动作而不是掉帧——序列帧导出要的就是这个。session 的时钟从此是时间轴的：`cycleData.elapsed` 和库的 `now` 是步长的累加（`simSeconds`），不读墙上时钟；暂停就是不步进。参数在 `_editorData.timeline`，动过才写进文档（不动就不出现在 COPY 里，round-trip 不变）。**拖播放头**（抓住那根线本身；点轨道空白处不动）= `session.holdPlayhead(frame)`：只挪时间位置，手里拿着的时候播放头钉在手下，松手从那里接着放；**模拟不倒带、不重算**，粒子该在哪还在哪（和 loop 不 restart 时转回开头是一回事）。第一版做成了"追"（回到开头不渲染地快进到目标帧，50 万粒子每秒约 170 帧），作者不要那个，删了，代码在 `022401d`。`length` 是整个项目的总帧数（默认 1200），轨道就是 0 … length，start / end 在里面选。暂停时 compute pass 也跳过（否则 kernel 会按上一步的 delta 继续推粒子，暂停停不住）。要让模拟本身回到某一帧得靠快照，和种子、导出一起记在根目录 `BACKLOG.md`。
- **做一个 example**（2026-09-18）：作者贴 JSON → 存成 `packages/engine/presets/examples/<slug>/config.json`（色源要是视频，`colorInstanceTextureId` 和 `_editorData.embeddedVideos` 指到站点自带的视频 URL，照 example-1-1；作者浏览器里的 `VideoTexture-…` id 别处解析不了）、`metadata.name` 改成新名字 → `presets.ts` 的 `examples` 加一项 → **缩略图按相机的画幅**：真实窗口里 `__studio.load(config)`，等粒子填满，`await __studio.capturePreview(384)` 拿 data URL 存成 `preview.webp`（引擎的 `captureOutput`，方相机就是方图；用 `packages/editor/scripts/cdp-eval.mjs` 取回来）→ `node packages/engine/scripts/sync-presets.mjs ../editor/public`。pieces 面板的卡片按缩略图自己的形状排（两列瀑布）。
