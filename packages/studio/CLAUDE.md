...
- **harness**：只在 dev bundle 里，页面 console 里 `await __st.report()`（67 条：boot、加载条、保存再读回、round-trip、schema 覆盖、live / rebuild 代价、家具、三个编辑器、演示、HUD、预览窗、面板）。真实窗口里跑更可靠，方法见 `packages/editor/CLAUDE.md` 的「验证改动」。
